...
async function prepararVenda(req,res){const vendaId=validId(req.params.venda_id);if(!vendaId)return res.status(400).json({message:"Venda inválida."});const client=await pool.connect();try{await client.query("BEGIN");
  ...
  const valorRecebidoDinheiro=pagamentos.filter(p=>p.forma_pagamento==="dinheiro").reduce((total,p)=>total+Number(p.valor||0),0);
  const payload={versao:"preparacao-v1",sem_emissao:true,alertas:[...new Set(alertas)],empresa,venda,cliente:{nome:venda.cliente,cpf:venda.cpf,cnpj:null,telefone:venda.telefone,email:venda.email},itens,pagamentos,totais:{subtotal:venda.subtotal,desconto:venda.desconto,frete:venda.frete_valor,total:venda.total,valor_recebido_dinheiro:valorRecebidoDinheiro,troco:venda.troco}};
