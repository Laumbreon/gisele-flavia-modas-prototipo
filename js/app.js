...
async function postClientesAuth(path,payload,mensagemPadrao){const response=await fetch(`${API_BASE_URL}/clientes-auth/${path}`,{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify(payload)});let data={};try{data=await response.json();}catch{data={};}if(!response.ok){const fallbacks={"cadastro:409":"Este e-mail já possui uma conta. Faça login ou use Esqueci minha senha.","login:401":"E-mail ou senha inválidos.","esqueci-senha:503":"Envio de e-mail ainda não configurado."};const mensagem=typeof data.message==="string"&&data.message.trim()?data.message.trim():fallbacks[`${path}:${response.status}`]||mensagemPadrao;throw new Error(mensagem);}return data;}
async function enviarAuthCliente(path,payload,errorSelector){const error=$(errorSelector);error.textContent="";try{const padrao=path==="cadastro"?"Não foi possível criar sua conta agora.":"Não foi possível entrar agora.";const data=await postClientesAuth(path,payload,padrao);salvarSessaoCliente(data.token,data.cliente);closeModal("#customerAuthModal");showToast(data.message||"Bem-vinda!");}catch(err){error.textContent=err.message||"Não foi possível conectar à loja. Tente novamente.";}}
...
async function solicitarCodigoCliente(event){event?.preventDefault();const email=$("#customerForgotEmail").value.trim(),error=$("#customerForgotError"),button=$("#customerForgotForm button[type='submit']");error.textContent="";button.disabled=true;try{const data=await postClientesAuth("esqueci-senha",{email},"Não foi possível enviar o código agora.");$("#customerResetEmail").value=email;$("#customerResetCode").value="";alternarAuthCliente("codigo");showToast(data.message);}catch(err){error.textContent=err.message||"Não foi possível conectar à loja. Tente novamente.";}finally{button.disabled=false;}}
async function redefinirSenhaCliente(event){event.preventDefault();const error=$("#customerResetError"),button=$("#customerResetForm button[type='submit']"),email=$("#customerResetEmail").value;error.textContent="";button.disabled=true;try{const data=await postClientesAuth("redefinir-senha",{email,codigo:$("#customerResetCode").value.trim(),novaSenha:$("#customerResetPassword").value,confirmarSenha:$("#customerResetConfirm").value},"Não foi possível redefinir a senha agora.");clienteSession=null;localStorage.removeItem(CLIENT_TOKEN_KEY);localStorage.removeItem(CLIENT_DATA_KEY);atualizarClienteUi();$("#customerLoginEmail").value=email;$("#customerLoginPassword").value="";alternarAuthCliente("login");showToast(data.message);setTimeout(()=>$("#customerLoginPassword").focus(),50);}catch(err){error.textContent=err.message||"Não foi possível conectar à loja. Tente novamente.";}finally{button.disabled=false;}}
...
function atualizarAuthUi() {
  ...
  if (logoutBtn) logoutBtn.hidden = !usuarioLogado() || clienteLogado();
...
async function carregarEstoqueDaApi() {
  ...
  try {
    const filtroProdutos=$("#stockV2ProductStatus")?.value||"ativos";
    const response = await fetchAdmin(`/estoque?produtos=${encodeURIComponent(filtroProdutos)}`);
    if (!response) return;
    if (!response.ok) throw new Error("HTTP " + response.status);
    const data = await response.json();
...
async function checkout() {
  ...
  try {
    ...
    cart=[];localShippingQuote=null;renderCart();updateCartBadge();$("#publicCheckoutContent").hidden=true;const mensagem=encodeURIComponent(`Olá! Acabei de fazer o pedido #${data.venda_id} pelo site.`),paymentLink=urlHttpSeguro(data.mercado_pago?.payment_link);$("#publicOrderSuccess").hidden=false;$("#publicOrderSuccess").innerHTML=`<div class="success-mark">✓</div><h3>Pedido recebido com sucesso!</h3><p>Número do pedido: <strong>#${data.venda_id}</strong></p><p>A loja entrará em contato para confirmar a entrega.</p>${paymentLink?`<div class="public-mp-payment"><strong>Pagamento Mercado Pago</strong><p>Use o ambiente seguro do Mercado Pago para concluir seu pagamento.</p><div><a class="btn btn-mp" href="${escaparHtml(paymentLink)}" target="_blank" rel="noopener">Pagar agora</a><button class="btn btn-ghost" id="publicCopyPaymentLink" type="button">Copiar link</button></div><small>Após o pagamento, a confirmação será atualizada automaticamente.</small></div>`:""}<a class="btn btn-pink" href="https://wa.me/5511999990000?text=${mensagem}" target="_blank" rel="noopener">Chamar no WhatsApp</a><button class="btn btn-ghost" id="publicContinueShopping" type="button">Continuar comprando</button>`;$("#publicContinueShopping").onclick=fecharCheckoutPublico;if($("#publicCopyPaymentLink"))$("#publicCopyPaymentLink").onclick=async()=>{const copiado=await copiarTextoSeguro(paymentLink);showToast(copiado?"Link copiado!":"Não foi possível copiar automaticamente.");};
    if (["cartao", "pix"].includes(payload.forma_pagamento)&&!paymentLink) $("#publicOrderSuccess").insertAdjacentHTML("beforeend", '<p class="mp-checkout-note">A loja enviará o link de pagamento Mercado Pago após conferir seu pedido.</p>');
...
function renderStockTable() {
  ...
  if (Array.isArray(estoqueApiRows)) {
    ...
    const produtosTotal=new Set(estoqueApiRows.map(r=>r.produto_id)).size,variacoesTotal=estoqueApiRows.filter(r=>r.variacao_id).length,baixos=estoqueApiRows.filter(r=>r.status==='baixo').length,zerados=estoqueApiRows.filter(r=>r.status==='zerado').length;$("#stockV2Summary").innerHTML=[["Produtos",produtosTotal],["Variações",variacoesTotal],["Estoque baixo",baixos],["Zerados",zerados]].map(([l,v])=>`<div><span>${l}</span><strong>${v}</strong></div>`).join('');
    const header = `<thead><tr><th>Produto</th><th>Variação</th><th>Estoque</th><th>Status</th><th>Ações</th></tr></thead>`;
    const rows = filtrados.map(row => {const arquivado=row.produto_status!=='ativo',temVariacao=Boolean(row.variacao_id);return `<tr class="${arquivado?'product-archived-row':''}" ${temVariacao?`data-stock-detail="${row.variacao_id}"`:''}><td data-label="Produto"><strong>${escaparHtml(row.produto_nome)}</strong>${arquivado?'<span class="product-archive-badge">Arquivado</span>':''}<small>${escaparHtml(row.categoria||"—")}</small></td><td data-label="Variação">${temVariacao?escaparHtml([row.tamanho,row.cor].filter(Boolean).join(" / ")):'Sem variações'}</td><td data-label="Estoque"><strong>${temVariacao?`${row.quantidade} un.`:'—'}</strong></td><td data-label="Status">${temVariacao?`<span class="tag-status ${row.status==='zerado'?'tag-zero':row.status==='baixo'?'tag-low':''}">${row.status==='zerado'?'Zerado':row.status==='baixo'?'Baixo':'Normal'}</span>`:'—'}</td><td data-label="Ações"><div class="stock-actions">${temVariacao?`<button class="btn btn-ghost btn-sm" data-detail-button="${row.variacao_id}" type="button">Ver detalhes</button>`:''}<button class="btn btn-ghost btn-sm" data-product-edit="${row.produto_id}" type="button">Editar</button>${arquivado?`<button class="btn btn-pink btn-sm" data-product-restore="${row.produto_id}" type="button">Restaurar</button><button class="btn btn-ghost btn-sm product-delete-action" data-product-delete="${row.produto_id}" data-product-name="${escaparHtml(row.produto_nome)}" type="button">Excluir</button>`:`<button class="btn btn-ghost btn-sm" data-product-archive="${row.produto_id}" data-product-name="${escaparHtml(row.produto_nome)}" type="button">Arquivar</button>${temVariacao?`<button class="btn btn-pink btn-sm" data-stock-move="${row.variacao_id}" data-stock-label="${escaparHtml(row.produto_nome)} — ${escaparHtml(row.tamanho)}/${escaparHtml(row.cor)}" type="button">Movimentar</button>`:''}`}</div></td></tr>`;}).join("");
    ...
    $$('[data-product-edit]',table).forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();openProductModal(Number(b.dataset.productEdit));}));
    $$('[data-product-archive]',table).forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();arquivarProdutoGestao(b);}));
    $$('[data-product-restore]',table).forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();restaurarProdutoGestao(b);}));
    $$('[data-product-delete]',table).forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();excluirProdutoGestao(b);}));
...
function rotuloStatusPedido(status) {
  return ({ criado:"Link gerado", pending:"Aguardando pagamento", pendente:"Pendente", in_process:"Em processamento", processando:"Em processamento", authorized:"Autorizado", approved:"Pago", pago:"Pago", rejected:"Rejeitado", cancelled:"Cancelado", cancelado:"Cancelado", refunded:"Estornado", charged_back:"Chargeback", erro_valor:"Divergência de valor", sem_entrega:"Sem entrega", separando:"Separando", pronto_retirada:"Pronto para retirada", saiu_entrega:"Saiu para entrega", entregue:"Entregue", mercado_pago:"Pago via Mercado Pago" })[status] || String(status || "—").replaceAll("_", " ");
}
...
async function abrirPedidoSite(id) {
  try {
    ...
    if (data.forma_pagamento === "mercado_pago") $("#siteOrderPay")?.remove();
    if (data.mercado_pago) {
      const mp=data.mercado_pago,aprovacao=mp.date_approved?new Date(mp.date_approved).toLocaleString("pt-BR"):"—";
      $(".site-order-actions", $("#siteOrderDetail"))?.insertAdjacentHTML("beforebegin", `<div class="mp-order-details"><strong>${data.status_pagamento==='pago'?'Pago via Mercado Pago':`Mercado Pago: ${escaparHtml(rotuloStatusPedido(mp.status))}`}</strong><span>Preference ID: ${escaparHtml(mp.preference_id||'—')}</span><span>Payment ID: ${escaparHtml(mp.payment_id||'—')}</span><span>Aprovação: ${escaparHtml(aprovacao)}</span>${mp.status==='erro_valor'?'<p>Pagamento recebido com divergência. Verifique o valor antes de confirmar.</p>':''}</div>`);
    }
    if (data.status_pagamento === "pendente" && !data.mercado_pago?.preference_id) {
    ...
    $(".site-order-actions", $("#siteOrderDetail"))?.insertAdjacentHTML("afterend", '<div class="mp-payment-result" id="siteOrderMpResult"><strong>Mercado Pago: link não gerado</strong><small>Pagamentos Mercado Pago são confirmados automaticamente após consulta oficial.</small></div>');
...
async function copiarTextoSeguro(texto){if(!texto)return false;try{if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(texto);return true;}}catch{}const area=document.createElement("textarea");area.value=texto;area.setAttribute("readonly","");area.style.position="fixed";area.style.opacity="0";document.body.appendChild(area);area.select();let copiado=false;try{copiado=document.execCommand("copy");}catch{}area.remove();return copiado;}
function urlHttpSeguro(valor){try{const url=new URL(String(valor||""));return ["http:","https:"].includes(url.protocol)?url.toString():"";}catch{return "";}}
function urlMercadoPago(data){return urlHttpSeguro(data?.url_pagamento||data?.sandbox_init_point||data?.init_point);}
function mostrarLinkMercadoPago(pedido,data){const box=$("#siteOrderMpResult");if(!box)return;const url=urlMercadoPago(data),status=data?.payment_status||data?.status||"criado",erroValor=status==='erro_valor'||data?.resultado_processamento==='divergencia_valor';const mensagem=`Olá! Segue o link de pagamento do seu pedido #${pedido.id} na Gisele Flávia Modas: ${url}`;box.innerHTML=`<strong>Mercado Pago: ${escaparHtml(rotuloStatusPedido(status))}</strong><span>Preference ID: ${escaparHtml(data?.preference_id||'—')}</span>${data?.payment_id?`<span>Payment ID: ${escaparHtml(data.payment_id)}</span>`:''}${data?.date_approved?`<span>Aprovação: ${new Date(data.date_approved).toLocaleString('pt-BR')}</span>`:''}${erroValor?'<p class="login-error">Pagamento recebido com divergência. Verifique o valor antes de confirmar.</p>':''}${url?`<a href="${escaparHtml(url)}" target="_blank" rel="noopener">${escaparHtml(url)}</a>`:""}<div>${url&&status!=='approved'?`<a class="btn btn-mp btn-sm" href="${escaparHtml(url)}" target="_blank" rel="noopener">Abrir link</a><button class="btn btn-ghost btn-sm" id="mpCopyLink" type="button">Copiar link</button><a class="btn btn-ghost btn-sm" href="${linkWhatsappPedidoSite(pedido,mensagem)}" target="_blank" rel="noopener">Enviar pelo WhatsApp</a>`:""}<button class="btn btn-ghost btn-sm" id="mpSyncPayment" type="button">Sincronizar pagamento</button></div><small>A confirmação automática só ocorre após consulta oficial do pagamento.</small>`;const generate=$("#siteOrderMercadoPago");if(generate&&data?.preference_id){generate.disabled=true;generate.textContent=status==='approved'?'Pagamento aprovado':'Link já gerado';}if($("#mpCopyLink"))$("#mpCopyLink").onclick=async()=>showToast(await copiarTextoSeguro(url)?"Link copiado!":"Não foi possível copiar automaticamente.");$("#mpSyncPayment")?.addEventListener("click",()=>sincronizarPagamentoMercadoPago(pedido.id));}
...
function dadosVariacaoEditor(item){const existente=Boolean(Number(item.dataset.variationId)),quantidade=Number($('.pv-stock',item).value||0);return{tamanho:$('.pv-size',item).value.trim(),cor:$('.pv-color',item).value.trim(),sku:$('.pv-sku',item).value.trim()||null,codigo_barras:$('.pv-barcode',item).value.trim()||null,preco_venda:$('.pv-price',item).value||null,preco_promocional:$('.pv-promo',item).value||null,...(existente?{quantidade_estoque:quantidade}:{quantidade_inicial:quantidade}),estoque_minimo:Number($('.pv-min',item).value||0),ativo:$('.pv-active',item).checked};}
...
async function atualizarSituacaoProdutoGestao(id,acao){const response=await fetchAdmin(`/produtos/${id}/${acao}`,{method:'PATCH'}),data=await response?.json().catch(()=>({}));if(!response?.ok)throw new Error(data.message||'Não foi possível atualizar o produto.');showToast(data.message);await carregarProdutosDaApi();await carregarEstoqueDaApi();if(typeof carregarProdutosEtiquetas==='function')carregarProdutosEtiquetas();}
async function arquivarProdutoGestao(button){if(!confirm(`Arquivar “${button.dataset.productName}”? O produto sairá da loja pública e do PDV, mas o histórico será preservado.`))return;button.disabled=true;try{await atualizarSituacaoProdutoGestao(Number(button.dataset.productArchive),'arquivar');}catch(error){showToast(error.message);}finally{button.disabled=false;}}
async function restaurarProdutoGestao(button){button.disabled=true;try{await atualizarSituacaoProdutoGestao(Number(button.dataset.productRestore),'restaurar');}catch(error){showToast(error.message);}finally{button.disabled=false;}}
async function excluirProdutoGestao(button){if(!confirm(`Excluir “${button.dataset.productName}” definitivamente? Se houver qualquer histórico, o sistema apenas arquivará o produto.`))return;button.disabled=true;try{const response=await fetchAdmin(`/produtos/${Number(button.dataset.productDelete)}`,{method:'DELETE'}),data=await response?.json().catch(()=>({}));if(!response?.ok)throw new Error(data.message||'Não foi possível excluir o produto.');showToast(data.message);await carregarProdutosDaApi();await carregarEstoqueDaApi();if(typeof carregarProdutosEtiquetas==='function')carregarProdutosEtiquetas();}catch(error){showToast(error.message);}finally{button.disabled=false;}}
...
function init() {
  ...
  $("#stockV2ProductStatus").addEventListener("change", carregarEstoqueDaApi);
