(function () {
  ...
  const pdvPaymentValue = () => document.querySelector('[name="forma_pagamento"]:checked')?.value || document.querySelector('[data-form="pdv-sale"] [name="forma_pagamento"]')?.value || "";
  ...
  function updatePdvCashPayment() {
    ...
    if (!form) return;
    const payment = pdvPaymentValue();
    const isCash = payment === "dinheiro";
    const isMixed = payment === "misto";
    ...
    const installmentsField = document.getElementById("pdvInstallmentsField");
  ...
  function preparePdvOperationalLayout() {
    const layout = document.querySelector(".pdv-layout");
    const checkout = document.querySelector(".pdv-checkout");
    const form = document.querySelector('[data-form="pdv-sale"]');
    if (!layout || !checkout || !form || document.querySelector(".pdv-payment-panel")) return;
    form.id = form.id || "pdvSaleForm";
    const paymentField = [...form.querySelectorAll(".field")].find((field) => field.querySelector(".payment-options"));
    if (!paymentField) return;
    paymentField.classList.add("pdv-payment-field");
    paymentField.querySelectorAll('[name="forma_pagamento"]').forEach((input) => input.setAttribute("form", form.id));
    const panel = document.createElement("section");
    panel.className = "section pdv-payment-panel";
    panel.innerHTML = "<h2>Forma de pagamento</h2>";
  ...
  function cashReportContentHtml(report = state.cashReport) {
    ...
    const caixas = report.caixas || (report.caixa ? [report.caixa] : []);
    const diferenca = caixas.reduce((sum, item) => sum + Number(item.diferenca || 0), 0);
    const valorInformado = caixas.reduce((sum, item) => sum + Number(item.valor_informado || 0), 0);
    const totalSistema = caixas.reduce((sum, item) => sum + Number(item.total_sistema || 0), 0);
    const machines = report.por_maquininha || [];
    return `<div class="cards cash-report-cards"><article class="card metric"><small>Total vendido</small><strong>${money(resumo.total_vendido)}</strong></article><article class="card metric"><small>Total sistema</small><strong>${money(totalSistema || resumo.total_vendido)}</strong></article><article class="card metric"><small>Valor informado</small><strong>${valorInformado ? money(valorInformado) : "Não informado"}</strong></article><article class="card metric ${Math.abs(diferenca) > 0.009 ? "cash-difference" : "cash-ok"}"><small>Diferença</small><strong>${Math.abs(diferenca) > 0.009 ? money(diferenca) : "Sem diferença"}</strong></article><article class="card metric"><small>Dinheiro</small><strong>${money(resumo.dinheiro)}</strong></article><article class="card metric"><small>Pix</small><strong>${money(resumo.pix)}</strong></article><article class="card metric"><small>Cartões</small><strong>${money(resumo.total_cartoes || Number(resumo.debito || 0) + Number(resumo.credito || 0))}</strong></article><article class="card metric"><small>Externos</small><strong>${money(resumo.total_externo)}</strong></article><article class="card metric"><small>Vendas</small><strong>${Number(resumo.quantidade_vendas || 0)}</strong></article></div>${cashReportBoxesHtml(caixas)}${cashReportAlertsHtml(report.alertas || [])}<div class="grid-two cash-report-grid"><section><h3>Totais por forma</h3><div class="table-wrap"><table><thead><tr><th>Forma</th><th>Qtd.</th><th>Total</th></tr></thead><tbody>${(report.por_forma_pagamento || []).map((item) => `<tr><td>${escapeHtml(paymentLabel(item.forma_pagamento))}</td><td>${Number(item.quantidade || 0)}</td><td>${money(item.total)}</td></tr>`).join("") || `<tr><td colspan="3">Sem vendas no período.</td></tr>`}</tbody></table></div></section><section><h3>Totais por maquininha</h3>${cashReportMachinesHtml(machines)}</section></div><h3>Vendas detalhadas</h3><div class="table-wrap"><table><thead><tr><th>Venda</th><th>Data</th><th>Cliente</th><th>Forma</th><th>Maquininha</th><th>Valor</th><th>Caixa</th><th>Operador</th><th>Status</th></tr></thead><tbody>${(report.vendas || []).map((item) => `<tr><td>#${item.id}</td><td>${date(item.created_at)}</td><td>${escapeHtml(item.cliente || "Consumidor não identificado")}</td><td>${escapeHtml(paymentLabel(item.forma_pagamento))}</td><td>${escapeHtml(item.maquininha || "Sem maquininha / Manual")}</td><td>${money(item.valor_pagamento || item.valor_relatorio || item.total)}</td><td>${item.caixa_id ? `#${item.caixa_id}` : "—"}</td><td>${escapeHtml(item.operador || "—")}</td><td>${escapeHtml(item.status_pagamento || item.status || "pago")}</td></tr>`).join("") || `<tr><td colspan="9">Nenhuma venda encontrada.</td></tr>`}</tbody></table></div>`;
  }

  function cashReportBoxesHtml(caixas) {
    if (!caixas.length) return `<div class="empty-state compact">Nenhum caixa vinculado às vendas filtradas.</div>`;
    return `<section class="cash-report-boxes"><h3>Dados do caixa</h3><div class="cash-box-list">${caixas.map((caixa) => `<article class="cash-box-card"><div><strong>Caixa #${caixa.id}</strong><span class="badge ${caixa.status === "aberto" ? "pending" : ""}">${caixa.status === "aberto" ? "Caixa aberto" : "Caixa fechado"}</span></div><p>Responsável: <b>${escapeHtml(caixa.responsavel || "—")}</b></p><p>Abertura: <b>${date(caixa.aberto_em)}</b></p><p>Valor inicial: <b>${money(caixa.valor_inicial)}</b></p><p>Fechamento: <b>${caixa.fechado_em ? date(caixa.fechado_em) : "Caixa em aberto"}</b></p><p>Valor informado: <b>${caixa.valor_informado == null ? "não informado" : money(caixa.valor_informado)}</b></p><p>Total sistema: <b>${money(caixa.total_sistema)}</b></p><p>Diferença: <b>${Math.abs(Number(caixa.diferenca || 0)) > 0.009 ? money(caixa.diferenca) : "Sem diferença"}</b></p>${caixa.observacoes_fechamento ? `<p>Obs.: ${escapeHtml(caixa.observacoes_fechamento)}</p>` : ""}</article>`).join("")}</div></section>`;
  }

  function cashReportAlertsHtml(alerts) {
    if (!alerts.length) return `<section class="cash-report-alerts is-ok"><h3>Alertas</h3><p>Sem alertas para os filtros selecionados.</p></section>`;
    return `<section class="cash-report-alerts"><h3>Alertas</h3>${alerts.map((alert) => `<p class="${alert.tipo === "atencao" ? "is-warning" : ""}">${escapeHtml(alert.mensagem)}</p>`).join("")}</section>`;
  }

  function cashReportMachinesHtml(machines) {
    if (!machines.length) return `<div class="empty-state compact">Sem pagamentos por maquininha no período.</div>`;
    return `<div class="cash-machine-list">${machines.map((machine) => `<article class="cash-machine-card"><header><strong>${escapeHtml(machine.maquininha_nome || machine.maquininha || "Sem maquininha / Manual")}</strong>${machine.codigo_externo ? `<small>${escapeHtml(machine.codigo_externo)}</small>` : ""}</header><div class="table-wrap"><table><thead><tr><th>Forma</th><th>Qtd.</th><th>Total</th></tr></thead><tbody>${(machine.formas || machine.linhas || []).map((item) => `<tr><td>${escapeHtml(paymentLabel(item.forma_pagamento))}</td><td>${Number(item.quantidade || 0)}</td><td>${money(item.total)}</td></tr>`).join("")}</tbody></table></div><footer><span>${Number(machine.quantidade || 0)} pagamento(s)</span><strong>${money(machine.total)}</strong></footer></article>`).join("")}</div>`;
  }
  ...
  function cashReportPrintHtml(report = state.cashReport) {
    if (!report) return "";
    return `<style>.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:14px 0}.card{border:1px solid #ddd;border-radius:8px;padding:10px}.card small{display:block;color:#555}.card strong{font-size:18px}.cash-report-alerts,.cash-box-card,.cash-machine-card{border:1px solid #ddd;border-radius:8px;padding:10px;margin:10px 0}.cash-report-alerts{background:#fffaf0}.cash-report-alerts.is-ok{background:#f3fff7}.cash-report-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.cash-machine-card footer,.cash-machine-card header,.cash-box-card>div{display:flex;justify-content:space-between;gap:10px}.report-subtotal td{font-weight:bold;background:#f7f7f7}@media print{@page{size:A4 portrait;margin:12mm}}</style><h1>Gisele Flávia Modas</h1><h2>Relatório de Caixa</h2><p>Período: ${escapeHtml(cashReportPeriod(report))}<br>Emitido em: ${date(new Date())}</p>${cashReportContentHtml(report)}`;
  }

  function cashReportThermalHtml(report = state.cashReport) {
    if (!report) return "";
    const resumo = report.resumo || {};
    const caixa = report.caixa || (report.caixas || [])[0] || {};
    const machines = report.por_maquininha || [];
    const valueLine = (label, value) => `<div><span>${escapeHtml(label)}</span><b>${money(value)}</b></div>`;
    const alerts = report.alertas || [];
    return `<style>.thermal-report{width:72mm;max-width:72mm;font-size:11px;line-height:1.25}.thermal-report h1{font-size:15px;margin:0 0 2mm;text-align:center}.thermal-report h2{font-size:12px;margin:0 0 2mm;text-align:center}.thermal-report h3{font-size:11px;margin:2mm 0 1mm}.thermal-report h4{font-size:10px;margin:2mm 0 1mm}.thermal-report div,.thermal-sale{display:flex;justify-content:space-between;gap:2mm}.thermal-report p{margin:1mm 0}.thermal-report hr{border:0;border-top:1px dashed #777;margin:2mm 0}@media print{@page{size:80mm auto;margin:0}}</style><div class="thermal-report"><h1>GISELE FLÁVIA MODAS</h1><h2>Relatório de Caixa</h2><p>Período: ${escapeHtml(cashReportPeriod(report))}<br>Emitido em: ${date(new Date())}</p><hr><h3>CAIXA</h3><div><span>Caixa</span><b>${caixa.id ? `#${caixa.id}` : "Vários"}</b></div><div><span>Status</span><b>${escapeHtml(caixa.status || "—")}</b></div><div><span>Abertura</span><b>${caixa.aberto_em ? date(caixa.aberto_em) : "—"}</b></div>${valueLine("Valor inicial", caixa.valor_inicial || resumo.valor_inicial || 0)}<div><span>Fechamento</span><b>${caixa.fechado_em ? date(caixa.fechado_em) : "Em aberto"}</b></div><div><span>Valor informado</span><b>${caixa.valor_informado == null ? "não informado" : money(caixa.valor_informado)}</b></div>${valueLine("Total sistema", caixa.total_sistema || resumo.total_vendido)}<div><span>Diferença</span><b>${Math.abs(Number(caixa.diferenca || 0)) > 0.009 ? money(caixa.diferenca) : "Sem diferença"}</b></div><hr><h3>ALERTAS</h3>${alerts.length ? alerts.map((alert) => `<p>- ${escapeHtml(alert.mensagem)}</p>`).join("") : "<p>Sem alertas.</p>"}<hr><h3>RESUMO</h3>${valueLine("Total vendido", resumo.total_vendido)}<div><span>Qtd. vendas</span><b>${Number(resumo.quantidade_vendas || 0)}</b></div><div><span>Qtd. itens</span><b>${Number(resumo.quantidade_itens || 0)}</b></div><hr><h3>FORMAS</h3>${["dinheiro","pix","debito","credito","vale_haver","cartao_solocard","cartao_brasil_card","cartao_asu"].map((key) => valueLine(paymentLabel(key), resumo[key])).join("")}<hr><h3>MAQUININHAS</h3>${machines.map((machine) => `<h4>${escapeHtml(machine.maquininha_nome || "Manual")}</h4>${(machine.formas || []).map((item) => valueLine(paymentLabel(item.forma_pagamento), item.total)).join("")}${valueLine("Total", machine.total)}`).join("") || "<p>Sem pagamentos.</p>"}<hr><h3>VENDAS</h3>${(report.vendas || []).map((item) => `<div class="thermal-sale"><span>#${item.id} ${date(item.created_at)} ${paymentLabel(item.forma_pagamento)} ${item.maquininha || "Manual"}</span><b>${money(item.valor_pagamento || item.total)}</b></div>`).join("") || "<p>Nenhuma venda.</p>"}</div>`;
  }
  ...
  function isPdvSaleControl(target, names) {
    return names.includes(target.name) && (target.closest('[data-form="pdv-sale"]') || target.getAttribute("form") === "pdvSaleForm");
  }
  ...
  function orderSeparationHtml(order) {
    ...
    const orderNotes = firstText(order.observacoes);
    const deliveryNotes = firstText(delivery.observacoes);
    ...
    const customerLines = [
      ...
      orderNotes ? `<p><strong>Observações do pedido:</strong> ${escapeHtml(orderNotes)}</p>` : "",
      deliveryNotes && deliveryNotes !== orderNotes ? `<p><strong>Observações da entrega:</strong> ${escapeHtml(deliveryNotes)}</p>` : "",
    ].filter(Boolean).join("");
  ...
  async function handleChange(event) {
    if (isPdvSaleControl(event.target, ["forma_pagamento", "maquininha_id", "parcelas"])) invalidatePdvPoint();
    if (isPdvSaleControl(event.target, ["forma_pagamento", "maquininha_id"])) updatePdvMachinePaymentUi();
    if (isPdvSaleControl(event.target, ["forma_pagamento", "valor_recebido_dinheiro", "pdv_desconto"])) updatePdvCashPayment();
